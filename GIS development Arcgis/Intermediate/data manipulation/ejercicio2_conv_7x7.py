...
#por cada fila y por cada columna restando el tamaño del kernel para la posicion final de f y c (deja un espacio de 7x7 al final del raster)
for f in range(filas-7):
    for c in range(columns-7):
        print(arrayb5[f:f+7, c:c+7])
        ...
        valor=(arrayb5[f:f+7, c:c+7]*kernel).sum()
        media=valor/49.0
...
raster_filtrado.save(ruta_out+'\\'+'filtro_7x7.tif')
